...
import os.path
import platform
from YEDDA_label_tool.utils.recommend import *
...
class Example(Frame):
    def __init__(self, parent):
        ...
        self.currentTriple= deque(maxlen=1) # store the current triple
        self.pressCommand = {'a':"Artifical",
                             ...
                             "8": "8",
                             "9": "9"
                             } ## relation and entity command
        self.allKey = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
        self.relationKey="0123456789so"
        self.controlCommand = {'q':"unTag", 'z':'undo','s':"subjectTag","o":"objectTag"}
        self.labelEntryList = []
    ...
    def initUI(self):
        ...
        for idx in range(0,len(self.relationKey)):
            press_key = self.relationKey[idx]
            self.text.bind(press_key, self.tripleTagging)
        ## relation tagging function bind

        self.text.bind('z', self.backToHistory)
    ...
    ## relation tagging function define
    def tripleTagging(self,event):
        command=event.char
        if self.debug: print("Action Track: tripleTagging -- "+command)
        try:
            content,cursor_index='',''
            if ord(command) >= 97:  ## command == 's' or 'o'
                first_index, last_index = self.text.index(SEL_FIRST), self.text.index(SEL_LAST)
                content,cursor_index = self.text.get("1.0", END),last_index
                current_entity = self.text.get(first_index, last_index)
                if not self.currentTriple:
                    if command =="s":
                        print("Subject is " + current_entity)
                        self.currentTriple.append([current_entity])
                    else: print("The operation doesn't fit the condition of the deque! The deque is now cleared!")
                elif len(self.currentTriple[0]) ==1 and command =="o":
                    print("Object is "+ current_entity)
                    self.currentTriple[0].append(current_entity)
                else:
                    print("The operation doesn't fit the condition of the deque! The deque is now cleared!")
                    self.currentTriple.pop()
            elif self.currentTriple and len(self.currentTriple[0])==2: ## command == 0-9 and deque is legal
                print("Relation is " + command)
                [subject,object]=self.currentTriple.pop()
                last_index=self.text.index(INSERT)
                cursor_index, triple = \
                    last_index.split('.')[0] + ".0", subject + "\t" + self.pressCommand[command] + "\t" + object + "\t\t"
                self.pushToHistory()
                self.text.insert(cursor_index,triple) ## output relation
                content, cursor_index = \
                    self.text.get("1.0", END), last_index.split('.')[0] + "." + str(len(triple) - 1)
            else:
                print("The operation doesn't fit the condition of the deque! The deque is now cleared!")
                self.currentTriple.pop()
                content, cursor_index = self.text.get("1.0", END), self.text.index(INSERT)
    ...
    def renewPressCommand(self):
        ...
        with open(self.configFile, 'w',encoding="utf-8") as fp:
            json.dump(self.pressCommand, fp,ensure_ascii=False)
        self.setMapShow()
