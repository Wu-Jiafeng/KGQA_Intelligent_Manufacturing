...
from tkinter.filedialog import askopenfilename, asksaveasfilename, askopenfilenames, asksaveasfile  # python3替换
...
from YEDDA_label_tool.utils.metric4ann import *
from YEDDA_label_tool.utils.compareAnn import *
...
class Example(Frame):
    ...
    def multiFiles(self):
        ...
        if len(filez) < 2:
            messagebox.showinfo("Monitor Error", "Selected less than two files!\n\nPlease select at least two files!")
        else:
    ...
    def compareTwoFiles(self):
        ...
        filez = askopenfilenames(parent=self.parent, filetypes = ftypes, title='Choose a file')
        if len(filez) != 2:
            messagebox.showinfo("Compare Error", "Please select exactly two files!")
        else:
            f = asksaveasfile(mode='w', defaultextension=".tex")
            write_result = compareBoundary(filez[0],filez[1],f)
            if write_result:
                messagebox.showinfo("Latex Generate", "Latex file generated successfully!\n\nSaved to "+ f.name)
                # import os
                # os.system("pdflatex "+ f.name)
            else:
                messagebox.showinfo("Latex Error", "Latex generated Error, two files don't have same sentence number!")
            f.close()
