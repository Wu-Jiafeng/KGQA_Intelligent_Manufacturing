...
import pyltp 
import os
from neo_db.config import cn
LTP_DATA_DIR = r'./KGQA/model'  # ltp模型目录的路径
...
def get_target_array(words):
    if not words: words="智能制造的特点是什么？"
    target_pos=['nh','n','ni','ns','nt','v']
    target_array=[]
    entities = cn.predict(words)
